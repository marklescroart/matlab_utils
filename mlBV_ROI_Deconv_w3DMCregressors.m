function mlBV_ROI_Deconv_w3DMCregressors(SubID,VTCs,RTCs,DATs,VOIfile,VOIname,ExpName,CondNames,CondCols,PrintToFile)

% usage: mlBV_ROI_Deconv_w3DMCregressors(SubID,VTCs,RTCs,DATs,VOIfile,VOIname,ExpName,CondNames,CondCols,PrintToFile)
%
% Inputs:
...
try
    ...
    ReduceOrNo = 0; % Done, this will give the number of 3x3x3 voxels you
                    % actually have - not done, it will give 1x1x1 voxels
                    % with redunant values (no averaging is actually done
                    % to get interpolated values for smaller voxels)

    Y = [];
    X = [];
    for iR = 1:nRuns
        MC = importdata(DATs{iR});                                         % Getting MC Data
        %load(matf{iR});
        ...
        TempX = RTC.RTCMatrix;
        TempX(:,end+1:end+3) = MC(end-size(TempX,1)+1:end,1:3);            % Adding Regressor
        %TempX(ToCut,:) = [];
