...
% StimPos (should be nTrials by 2 for each image flash)
% 
...
for iTrial = 1:nTrials;
    ...
    % First: Mean eye position:
    xx = mean(ET(TrIndex,1));
    yy = mean(ET(TrIndex,2));
    
    distFromFix = sqrt((xx-512)^2+(yy-384)^2);
    if distFromFix > 42
        fprintf('Trial %.0f had a mean eye position away from fixation.\n',iTrial)
    end
