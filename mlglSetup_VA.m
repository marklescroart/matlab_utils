function varargout = mlglSetup_VA(GLst,View)

% Usage: [win,ScrVars,GLCol,GLst,View] = mlOpenGLSetup([GLst] [,View])
% 
...
if ~exist('Glst','var') || isempty(GLst)
    ...
    %%% Lighting:
    % Ambient first: 
    GLst.AmbientLight   = [1 1 1]; %.2 * GLCol.White; % usually .2 % TEMPORARY! Going with ambient light only (spotlights complicate things)
%{    
    % Light 1: 
    GLst.Light(1).Pos      = [0,0,10,1];
    GLst.Light(1).Color    = 1 * GLCol.White;
    GLst.Light(1).SpotExp = 0; % was 64 % Default is 0 - that is, diffuse, not focused
    GLst.Light(1).SpotCut = 180; %60; % Default is 180 - that is, pointed every which-way
...
%     GLst.Light(2).SpotExp = 64;
    ...
    %%% Perspective:
    GLst.fovY = 8.97;  % Field of view is +/- 45 degrees from line of sight.
